/* Scene/Kunst gallery. Vertical stretch and horizontal image-based morph.
   Morph adapted from paniq's MIT-licensed GL Transitions shader.
...
const loadData = () => dataPromise ||= fetch(new URL('./timeline-gallery-data.json?v=20260925', import.meta.url)).then(r => {
...
// Crop only when the detected faces fit together. Otherwise show the whole image.
...
export function cropRect(iw, ih, w, h, meta, fill) {
  ...
  if (faces.length) {
    ...
    if (right - left > cw || bottom - top > ch) return null;
    x = clamp((left + right - cw) / 2, right - cw, left);
    y = clamp((top + bottom) / 2 - ch * .36, bottom - ch, top);
  }
  return [clamp(x, 0, iw - cw), clamp(y, 0, ih - ch), cw, ch];
}

// Use every photo's aspect ratio with equal weight, irrespective of resolution.
// This keeps a series stable while making landscape-only series landscape.
export function galleryRatio(items, media) {
  const ratios = items.map(src => media[src]).filter(m => m?.width > 0 && m?.height > 0).map(m => m.width / m.height);
  return ratios.length ? Math.exp(ratios.reduce((sum,r) => sum + Math.log(r),0) / ratios.length) : 4 / 3;
}
...
function compose(image, w, h, meta, fill) {
  ...
  const sameOrientation = (image.naturalWidth >= image.naturalHeight) === (w >= h);
  const crop = cropRect(image.naturalWidth,image.naturalHeight,w,h,meta,fill && !sameOrientation);
  if (crop) ctx.drawImage(image,...crop,0,0,w,h);
  else {
...
function createTransition(canvas) {
  ...
  return {
    prepare(before, after, effect, direction) {
      ...
      gl.uniform1f(gl.getUniformLocation(program, 'effect'), effect === 'live' ? 0 : 1);
...
export function openTimelineGallery(entry, group, theme = {}) {
  ...
  let seriesRatio=4/3, hasPortrait=false, hasLandscape=false;
  ...
  function layout() {
    ...
    const maxW=Math.min(vw-margin*2,1800);
    // Mixed mobile series follow the available orientation. Uniform series
    // keep their own format, including landscape-only sets on portrait phones.
    let ratio=seriesRatio;
    if(compact && hasPortrait && hasLandscape) ratio=vw>vh ? Math.max(ratio,16/9) : Math.min(ratio,3/4);
    dialog.style.setProperty('--tg-ratio',String(ratio));
    let width=maxW;
    for(let i=0;i<4;i++) {
      dialog.style.width=`${width}px`;
      const available=Math.max(160,vh-margin*2-header.offsetHeight);
      const nextWidth=Math.max(Math.min(maxW,360),Math.min(maxW,available*ratio));
  ...
  function beginScene(direction) {
    ...
    imageAt(session.target).then(image => {
      ...
      session.after = compose(image,w,h,meta,mobile.matches && hasPortrait && hasLandscape && Boolean(meta.faces?.length));
      if (drawing && drawing.width === w && drawing.height === h && !reduced.matches) {
        transition ||= createTransition(effectCanvas);
        if (transition) {
          transition.prepare(drawing,session.after,'live',direction);
  ...
  async function render(direction = 0) {
    ...
    try {
      ...
      const composed = compose(image,w,h,meta,mobile.matches && hasPortrait && hasLandscape && Boolean(meta.faces?.length));
      ...
      if (direction && before && before.width === w && before.height === h && !reduced.matches) {
        transition ||= createTransition(effectCanvas);
        if (transition) {
          transition.prepare(before,composed,group,direction); transition.draw(0);
  ...
  function populate(data) {
    ...
    if (entry.href) link('Se det oprindelige arkiv ↗',entry.href);
    if (items.length) {
      seriesRatio=galleryRatio(items,media);
      hasPortrait=items.some(src=>media[src]?.height>media[src]?.width);
      hasLandscape=items.some(src=>media[src]?.width>media[src]?.height);
